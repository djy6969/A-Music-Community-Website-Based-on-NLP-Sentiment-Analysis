...
def searchMusic():
    ...
    musics = {}
    musics_info = TMusic.query.filter(TMusic.title.like('%{keyword}%'.format(keyword=searchContent))).order_by(TMusic.publish_time.desc()).all()
    for i in range(len(musics_info)):
        ...
        music_i_info = {'seq': i, 'id': music_id, 'img': music_info.image_url, "url": music_file_path, 'name': music_info.title,
