<template>
  <div class="page-wrapper">
    <HeaderViewSection />
    <section class="about-area-four pt-25 rpt-0 rel z-2">
      <div class="container">
        <div class="row align-items-center">
          <div
            class="about-four-content mb-65 rel z-1 wow fadeInLeft delay-0-2s"
          >
            <ul class="nav nav-pills nav-fill mb-35">
              <li class="nav-item">
                <a
                  class="nav-link active"
                  data-bs-toggle="tab"
                  href="#about-tap1"
                  >User Login</a
                >
              </li>
              <li class="nav-item">
                <a class="nav-link" data-bs-toggle="tab" href="#about-tap2"
                  >Clerk Login</a
                >
              </li>
            </ul>
            <div class="tab-content">
              <div class="tab-pane fade show active" id="about-tap1">
                <UserLoginSection />
              </div>
              <div class="tab-pane fade" id="about-tap2">
                <ClerkLoginSection />
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  </div>
</template>

<script>
import UserLoginSection from "@/views/LoginView/UserLoginSection.vue";
import ClerkLoginSection from "@/views/LoginView/ClerkLoginSection.vue";
import HeaderViewSection from "@/views/LoginView/HeaderViewSection.vue";

export default {
  name: "LoginView",
  components: { HeaderViewSection, ClerkLoginSection, UserLoginSection },
};
</script>

<style scoped></style>
